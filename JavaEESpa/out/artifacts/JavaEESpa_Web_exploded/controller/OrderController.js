...
function ddlselect() {
    ...
    for (let i = 0; i < customerDB.length; i++) {
        if (customerDB[i].id == display) {
            ...
            orderDB.splice(0, orderDB.length);
            validateCustomer();
        }
    }
    getId();
    ...
    $.ajax({
        ...
        success: function (resp) {
            for (const order of resp.data) {
                $("#exampleInputEmail40").val(order.id);
            }
        }
    });
/*
    if (orderDetailDB.length > 0) {
        var valu2 = orderDetailDB[orderDetailDB.length - 1].code;
...
/*function valDate() {
    var regExdate = /^(0[1-9]|1[0-2])\/(0[1-9]|1\d|2\d|3[01])\/(0[1-9]|1[1-9]|2[1-9])$/;
    let input = $("#exampleInputEmail4").val();
    if (regExdate.test(input)) {
        $("#exampleInputEmail4").css('border', '2px solid green');
        date = 1;
        enableItemBtn();
    } else {
        $("#exampleInputEmail4").css('border', '2px solid red');
        date = 0;
        enableItemBtn();
    }
...
/*var regExdate2 = /^([0-2][0-9]|(3)[0-1])(\/)(((0)[0-9])|((1)[0-2]))(\/)\d{4}$/;
$("#exampleInputEmail4").keyup(function () {
    let input = $("#exampleInputEmail4").val();
    if (regExdate2.test(input)) {
        $("#exampleInputEmail4").css('border', '2px solid green');
        date = 1;
...
function searchOrder() {
    ...
    var searchID = $("#exampleInputEmail40").val();
    ...
    $.ajax({
        ...
        success: function (resp) {
            $("#orderTbl").empty();
            for (const order of resp.data) {
                let row = `<tr><td>${order.code}</td><td>${order.name}</td><td>${order.price}</td><td>${order.qty}</td><td>${order.total}</td></tr>`;
                $("#orderTbl").append(row);
            }
        }
...
function validateOId() {
    ...
    let input20 = $("#exampleInputEmail40").val();
    if (regExOrderID.test(input20)) {
        $("#exampleInputEmail40").css('border', '2px solid green');
        orderCod5 = 1;
        enableBtn();
    } else {
        $("#exampleInputEmail40").css('border', '2px solid red');
        orderCod5 = 0;
        enableBtn();
    }
}
...
function enableItemBtn() {
    ...
    var isTrue = (date + itemCod2 + itemNam2 + itemQt2 + itemPric2 + itemQt + cuId2 + cuName2 + cuAddress2 + cuTp2 + OId2) == 10;
    if (isTrue) {
...
function saveOrderDetail() {
    ...
    for (var i = 0; i < orderDB.length; i++) {
        ...
        var orderOb = {
            ...
            name: orderDB[i].name,
            price: parseInt(orderDB[i].price),
            qty: parseInt(orderDB[i].qty),
            total: parseInt(orderDB[i].total)
        };
...
var regExOrderCode = /^(O00-)[0-9]{3,4}$/;
$("#exampleInputEmail40").keyup(function () {
    let input = $("#exampleInputEmail40").val();
    if (regExOrderCode.test(input)) {
        $("#exampleInputEmail40").css('border', '2px solid green');
        orderCod5 = 1;
        enableItemBtn();
    } else {
